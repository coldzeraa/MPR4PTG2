...
export const generatePdfContent = async (exID) => {
  ...
  const response = await fetch(
    ...
    }/api/get_${exName}_pdf?id=${encodeURIComponent(exID)}`,
    {
      method: "GET",
      headers: {
...
async function getExaminationType(exID) {
  const response = await fetch(
    `${
      process.env.REACT_APP_API_URL
    }/api/get_examination_type?id=${encodeURIComponent(exID)}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
...
async function fetchHtmlContent(exID) {
  const response = await fetch(
    `${
      process.env.REACT_APP_API_URL
    }/api/get_ishihara_pdf?id=${encodeURIComponent(exID)}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
  ...
  if (response.ok) {
    ...
    throw new Error(
      `Failed to fetch HTML content. Status: ${response.status} ${response.statusText}`
    );
  }
}
...
function PdfDownload({ exID }) {
  ...
  useEffect(() => {
    const initialFirstDownload =
      localStorage.getItem("firstDownload") === "true";
  ...
  const handlePdfDownload = async () => {
    try {
      ...
      let exType = await getExaminationType(exID);
      let exName = exType === "P" ? "Perimetrie" : "Ishihara";

      let fileName = exName + "_Ergebnis.pdf";
  ...
  return (
    <button
      className={
        "button " + (firstDownload && skipButton ? "button-disabled" : "")
      }
